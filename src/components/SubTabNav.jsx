    ...
    <div className={`inline-flex rounded-xl border border-slate/25 bg-canvas-dark/70 p-1 gap-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] ${fullWidth ? 'w-full' : 'w-fit'}`}>
      {normalizedTabs.map((tab) => (
        <button
          ...
          className={`ss-btn outline ${tab.icon ? 'icon-toggle' : ''} flex-1 whitespace-nowrap text-center px-4 py-2 text-sm font-semibold rounded-lg border transition-all duration-150 ${active === tab.value ? 'is-active' : ''} ${buttonClassName}`}
          aria-label={tab.label}
          title={tab.label}
          aria-pressed={active === tab.value}
          style={{ minWidth: minButtonWidth, ...buttonStyle }}
        >
