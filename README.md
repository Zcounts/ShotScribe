...
- Storyboard shot labels now use the scene Shot Number Prefix with centralized period formatting (for example `7B.T - Camera 1`) across cards, dialogs, references, mobile packages, and exports. Empty prefixes fall back to the normalized scene number/label before scene order, preserving the existing generated suffix order and skip-letter behavior.
