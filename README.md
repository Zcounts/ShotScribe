...
- Storyboard browser-fallback PDF export now preserves per-page width/height ordering when creating jsPDF pages, preventing sideways 90°-rotated page content and keeping portrait/landscape output aligned with the storyboard layout.
- Storyboard interactions now avoid accidental scene-properties opens on double-click, scene/shot context menus use explicit "Open Scene Properties"/"Open Shot Properties" labels, and storyboard scene header `SCENE #` + `Title / Slugline` fields support direct inline editing with Enter/blur commit and Escape cancel.
- Storyboard shot labels now use the scene Shot Number Prefix with centralized period formatting (for example `7B.T - Camera 1`) across cards, dialogs, references, mobile packages, and exports, while preserving the existing generated suffix order and skip-letter behavior. Shot-card right-click menus are restored across the full card surface without suppressing scene right-click menus.
...
- Storyboard Scene Properties slugline fix (May 27, 2026): storyboard scenes now always allow manual `Title / Slugline` edits (linked or unlinked), preserve overrides across modal reopen/save cycles and unrelated metadata edits, and only auto-refresh slugline from a newly linked script scene when no manual override exists.
